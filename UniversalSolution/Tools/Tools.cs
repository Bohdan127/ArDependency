...
namespace Tools
{
    ...
    public static class Tools
    {
        ...
        public delegate void EnumMyControlsCallback(Control c);

        public static void EnumMyControls(System.Windows.Forms.Control parent, EnumMyControlsCallback callback)
        {
            foreach (Control c in parent.Controls)
            {
                callback(c);
                EnumMyControls(c, callback);
            }
        }
        ...
        public class FormatterKeyPad : IFormatProvider, ICustomFormatter
        {
            ...
            public string ConvertToDBFormat(Keys arg)
            {
                string result = arg.ToString();
                string tmp = arg.ToString();
                if ((arg & Keys.Shift) == Keys.Shift)
                {
                    tmp = tmp.Replace("ShiftKey", "");
                ...
                else if (result == "OemMinus")
                {
                    result = "-";
                }
                else if (result == "OemQuestion")
                {
                    result = "/";
                }
                else if (result.Length == 2 && result.StartsWith("D"))
                {
            ...
            public string Format(Keys arg)
            {
                string result;
                string keyname;

                keyname = ConvertToDBFormat(arg);
                result = Format(keyname);

                return result;
            }

            public bool IsCorrectKeyPadValue(Keys arg)
            {
                string keyname = ConvertToDBFormat(arg);

                return IsCorrectKeyPadValue(keyname);
            }
        ...
        static public void AppendRtfTextToRtf(RichTextBox richTextBox, string appendText, string appendBefore)
        {
            string text = string.Empty;
            string rtf = string.Empty;
            int index = 0;
            if (null != richTextBox && appendText != null)
            {
                text = richTextBox.Text;
                if (appendBefore != null && appendBefore != string.Empty)
                {
                    text = appendBefore;
                }
                text = text.Trim('\n');
                if (appendBefore != string.Empty && text.IndexOf("\n") > 0)
                {
                    text = text.Substring(0, text.IndexOf("\n"));
                }
                index = richTextBox.Rtf.IndexOf(text);
                if (index > 0)
                {
                    rtf = richTextBox.Rtf;
                    rtf = rtf.Insert(index, appendText);
                    richTextBox.Rtf = rtf;
                }
            }
        }
        ...
        /// <summary>
        /// Appends text to RTF control
        /// </summary>
        /// <param name="richTextBox"></param>
        /// <param name="appendText"></param>
        /// <returns></returns>
        static public void AppendText(RichTextBox richTextBox, string appendText)
        {
            if (null != richTextBox && appendText != null)
            {
                richTextBox.Select(richTextBox.Text.Length, 0);
                richTextBox.SelectedText = appendText;
                richTextBox.Select(0, 0);
            }
        }

        /// <summary>
        /// Appends RTF string to RTF control
        /// </summary>
        /// <param name="richTextBox"></param>
        /// <param name="appendRtf"></param>
        /// <returns></returns>
        static public void AppendRtf(RichTextBox richTextBox, string appendRtf)
        {
            if (null != richTextBox && appendRtf != null)
            {
                richTextBox.Select(richTextBox.Text.Length, 0);
                richTextBox.SelectedRtf = appendRtf;
                richTextBox.Select(0, 0);
            }
        }

        /// <summary>
        /// Appends RTF string to RTF control with Text background Highlight
        /// </summary>
        /// <param name="richTextBox"></param>
        /// <param name="appendRtf"></param>
        /// <returns></returns>
        static public void AppendRtfWithHighlight(RichTextBox richTextBox, string appendRtf)
        {
            using (RichTextBox rTB = new RichTextBox())
            {
                if (null != richTextBox && appendRtf != null)
                {
                    if (IsRtfString(appendRtf.ToString()) == false)
                    {
                        rTB.Text = appendRtf.ToString();
                        appendRtf = rTB.Rtf;
                    }
                    appendRtf = System.Text.RegularExpressions.Regex.Replace(appendRtf, @"(\\cb)(\d+)\\", "\\highlight$2\\");
                    richTextBox.Select(richTextBox.Text.Length, 0);
                    richTextBox.SelectedRtf = appendRtf;
                    richTextBox.Select(0, 0);
                }
        ...
        // OnCheckNumericValue - KeyPress event handler
        // eg.
        // textEdit.KeyPress += new System.Windows.Forms.KeyPressEventHandler(OnCheckNumericValue);
        //
        static public void OnCheckNumericValue(object sender, System.Windows.Forms.KeyPressEventArgs e)
        {
            string decimalPoint = System.Globalization.CultureInfo.CurrentCulture.NumberFormat.NumberDecimalSeparator;

            if (!(char.IsControl(e.KeyChar) || char.IsDigit(e.KeyChar) || (decimalPoint.IndexOf(e.KeyChar) != -1)))
                e.Handled = true;
        }
